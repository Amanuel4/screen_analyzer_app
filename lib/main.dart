...
class _AnalyzerHomePageState extends State<AnalyzerHomePage> {
  ...
  String _aiResponseText = "Welcome! How can I help you with your screen today?";
  ...
  Future<void> _captureOrPickScreen(ImageSource source, String taskKey) async {
    ...
    XFile? imageFile;
    if (source == ImageSource.camera) { // Simulate full screen capture by picking from gallery
        // In a real app with native code, this would be MediaProjection.
        // For this demo, we use image picker as a stand-in.
        imageFile = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 70);
        if (imageFile != null && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Simulating full screen capture with selected image.")));
        }
    ...
    if (imageFile != null) {
      ...
      if (mounted) {
        setState(() {
          ...
          _displayedImageSize = Size(decodedImage.width.toDouble(), decoded_image.height.toDouble());
  ...
  Future<void> _analyzeImageWithGemini(Uint8List imageBytes, String taskKeyOrCustomPrompt) async {
    String geminiTaskPrompt = "";
    // Construct prompt based on taskKeyOrCustomPrompt
    if (taskKeyOrCustomPrompt == "data_usage") {
        geminiTaskPrompt = "This is an image from a smartphone screen. User wants to understand data usage. Analyze to identify: 1. Total data used & period. 2. Top apps by usage. 3. Data limits/warnings. Summarize. If info isn't visible, state that clearly.";
    } else if (taskKeyOrCustomPrompt == "data_saver") {
        geminiTaskPrompt = "This is an image from a smartphone screen. User wants to find 'Data Saver' settings. 1. Is it visible? 2. ON/OFF? 3. Based ONLY on what's visible, suggest how to navigate/toggle. If not visible, suggest common locations.";
    } else if (taskKeyOrCustomPrompt == "generic_what_is_this") {
        geminiTaskPrompt = "This is an image from a smartphone screen. Describe main elements, likely app/page, and its primary purpose based on visual content.";
    } else { // Assume it's a custom prompt
    ...
    if (result['error'] != null) {
      ...
      _parseAnalysisForDrawing(result['analysis_text']); // Try to find elements to draw
    } else {
      _updateAiResponse("Received an empty or unexpected response from analysis.");
    }
  ...
  void _parseAnalysisForDrawing(String analysisText) {
    ...
    if (analysisText.toLowerCase().contains("data saver")) {
        ...
        newObjects.add(DetectedObject(
            boundingBox: const Rect.fromLTWH(0.2, 0.3, 0.6, 0.1), // Example: 20% from left, 30% from top, 60% width, 10% height
            label: "Data Saver Area (Example)",
            color: Colors.orangeAccent));
    }
    if (analysisText.toLowerCase().contains("data usage is") && analysisText.toLowerCase().contains("gb")) {
         newObjects.add(DetectedObject(
  ...
  void _sendChatInput() { // This will be used for triggering analysis based on text for now
    ...
    } else if (text.toLowerCase().contains("analyze screen") || text.toLowerCase().contains("what is this")) {
      _showActionChoiceDialog("generic_what_is_this");
    }
    else {
      _updateAiResponse("I can help analyze your screen for tasks like 'check data usage' or 'find data saver'. Please share or upload your screen.");
    }
    ...
    showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: const Text("How would you like to provide the screen content?"),
            actions: <Widget>[
              TextButton(
                child: const Text("Live Screen Capture (Simulated)"),
                onPressed: () {
                  Navigator.of(context).pop();
                  _captureOrPickScreen(ImageSource.camera, taskKey); // Simulates live capture
                },
              ),
              TextButton(
  ...
  void _showStreamOptions() {
    ...
    print("Stream options icon clicked - integrate modal here if needed.");
     ...
     _showActionChoiceDialog("generic_what_is_this"); // Default task if icon clicked
  }
        ...
        child: Column(
          children: <Widget>[
            ...
            Container(
              ...
              child: _currentDisplayImage != null && _uiImageForOverlay != null
                  ? ClipRRect(
                      borderRadius: BorderRadius.circular(7),
                      child: DrawingOverlay( // Use the CustomPaint widget
                          backgroundImage: _uiImageForOverlay!,
                          detectedObjects: _detectedObjectsForOverlay,
                          imageSize: _displayedImageSize,
                      )
                    )
                  : Center(child: Text("Share or upload screen to see preview", style: TextStyle(color: Colors.white54))),
            ),
            ...
            Row(
                ...
                children: [
                    ElevatedButton.icon(
                        ...
                        onPressed: _isProcessing ? null : () => _showActionChoiceDialog("generic_what_is_this"),
                    ),
                    ElevatedButton.icon(
                        icon: const Icon(Icons.photo_library),
                        label: const Text("Upload Image"),
                        onPressed: _isProcessing ? null : () => _captureOrPickScreen(ImageSource.gallery, "generic_what_is_this"),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: "Type command (e.g., 'check data usage')...",
            ...
            Expanded(
              child: Container(
                ...
                child: _isProcessing
                    ...
                    : SelectableText(_aiResponseText, textAlign: TextAlign.left),
              ),
            ),
          ],
        ),
